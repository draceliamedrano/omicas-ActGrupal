...
#Redondeamos los valores para su análisis por DESeq2
...
#Realizamos el análisis con DESeq2
...
dds <- DESeq(dds,
             fitType = "glmGamPoi")
res <- results(dds,
               contrast = c("condition", "Obeso", "Normopeso"))
...
res_df$significant <- ifelse(
  res_df$padj < 0.05 & abs(res_df$log2FoldChange) > 0.5,
  "sig",
  "no_sig"
) # Se consideran significativo aquellos genes cuyo valor de logFC > 1 y p.adj < 0.05
...
p1 <- ggplot(df_gsk, aes(x = NES, y = Description)) +
  ...
  theme(axis.text.y = element_text(size = 14),
        axis.text.x = element_text(size = 14)) # No hay enriquecimiento significativo en valores KEGG
...
(p1 / p2 / p3)
write.csv(top_genes, file = "Genes Significativos 2.csv")

#Representamos un net plot del resultado del GSEA hecho sobre los Hallmarks de Reactome
ego2 <- setReadable(
  gsea_reactome,
  OrgDb = org.Hs.eg.db,
  keyType = "ENTREZID"
...
#Tabla csv con los resultados del DESeq2 solo para los genes significativos
top_genes <- top_genes %>%
          dplyr::select(-"gene", -"significant")

write.csv(top_genes, file = "Resultados DESeq2 significativos.csv")
